 ...
 */ const useInsertionEffect = typeof window !== "undefined" ? React__default.default.useInsertionEffect : noop;
/**
        ...
        try {
            const res = await uploadFiles(endpoint, {
                ...
                onUploadProgress: (progress)=>{
                    ...
                    const averageProgress = shared.roundProgress(Math.min(100, sum / fileProgress.current.size), progressGranularity);
 ...
 */ function UploadButton(props) {
    ...
    const $props = props;
    const { mode = "auto", appendOnPaste = false, cn = shared.defaultClassListMerger } = $props.config ?? {};
    ...
    const { startUpload, isUploading, routeConfig } = __useUploadThingInternal(shared.resolveMaybeUrlArg($props.url), $props.endpoint, $props.fetch ?? globalThis.fetch, {
    ...
    const inputProps = React.useMemo(()=>({
            ...
            ref: fileInputRef,
            multiple,
            accept: shared.generateMimeTypes(fileTypes).join(", "),
            onChange: (e)=>{
    ...
    const renderButton = ()=>{
        ...
        switch(state){
            ...
            case "uploading":
                {
                    if (uploadProgress >= 100) return /*#__PURE__*/ jsxRuntime.jsx(Spinner, {});
                    return /*#__PURE__*/ jsxRuntime.jsxs("span", {
                        ...
                        children: [
                            ...
                            /*#__PURE__*/ jsxRuntime.jsx(Cancel, {
                                cn: cn,
    ...
    const renderClearButton = ()=>/*#__PURE__*/ jsxRuntime.jsx("button", {
            onClick: ()=>{
                setFiles([]);
                if (fileInputRef.current) {
                    fileInputRef.current.value = "";
            ...
            className: cn("h-[1.25rem] cursor-pointer rounded border-none bg-transparent text-gray-500 transition-colors hover:bg-slate-200 hover:text-gray-600", shared.styleFieldToClassName($props.appearance?.clearBtn, styleFieldArg)),
            style: shared.styleFieldToCssObject($props.appearance?.clearBtn, styleFieldArg),
            "data-state": state,
            "data-ut-element": "clear-btn",
            children: shared.contentFieldToContent($props.content?.clearBtn, styleFieldArg) ?? "Clear"
        });
    const renderAllowedContent = ()=>/*#__PURE__*/ jsxRuntime.jsx("div", {
            className: cn("h-[1.25rem] text-xs leading-5 text-gray-600", shared.styleFieldToClassName($props.appearance?.allowedContent, styleFieldArg)),
